...
parser$add_argument('-refpt', '--refpt', type='character', nargs="+")
...
parser$add_argument('-y', '--assay', type='character', nargs="+")
...
melt = function(inmatrix, refpt, group, sample, annotation, assay, binsize, upstream, outpath){
    ...
    if(binsize>1){
        df = df %>%
            transmute(group = group, sample = sample,
                      annotation = annotation, assay = assay,
...
melt(inmatrix = args$input,
     ...
     assay = paste(args$assay, collapse=" "),
